                ...
                {studentData?.map((item, index) => {
                  return (
                    <Tr key={item?._id} className="capitalize">
                      ...
                      <Td>{item?.collegeId ? item?.collegeName : "--"}</Td>
                      <Td>
                        {item?.departmentName ? item?.departmentName : "--"}
                      </Td>
                      <Td>{item?.batchId ? item?.batchName : "--"}</Td>
                      <Td>{item?.courseId ? item?.courseName : "--"}</Td>
                      <Td>
                        {item?.trackingStatus ? "Enrolled" : "UnEnrolled"}
                      </Td>
                      <Td>
                        {item?.trackingStatus
                          ? item?.totalcontent === 0
                            ? "0%"
                            : item?.trackingContent === 0
                            ? "0%"
                            : `${(
                                (item.trackingContent / item.totalcontent) *
                                100
                              ).toFixed(2)}%`
                          : "--"}
                      </Td>
