  ...
  const handleResultSubmit = async () => {
    try {
      const response = await axiosInstance.put(
        `user/tracking-exercise-result/${trackingId}`,
        {
          totalAssignMarks: finalAssignMarks,
          result: updateResult
        }
      );
      if (response?.success) {
        openNotification("success", response?.message);
        navigate("/app/admin/assessment-exercise-result");
        setApiFlag(true);
      } else {
        openNotification("danger", response.message?.message);
      }
                            ...
                            <div className="flex gap-4 items-centermt-2 p-4 bg-blue-50 border border-blue-300 rounded-lg">
                              ...
                              <InputGroup className="">
                                <FormNumericInput
                                  ...
                                  disabled={!isValidUrl(question.answer)}
                                  min={0}
                                  max={question.marks}
                                  onBlur={(e) => {
                                    ...
                                    const tempData = [...updateResult];
                                    tempData[index] = {
                                      ...updateResult[index],
                                      assignMasks: value
                                    };
                                    setupdateResult(tempData);
                                  }}
                                  onChange={(e) => {
                                    if (
                                      Number(e.target.value) <= question.marks
                                    ) {
                                      const tempData = [...updateResult];
                                      tempData[index] = {
                                        ...updateResult[index],
                                        assignMasks: Number(e.target.value)
                                      };
                                      setupdateResult(tempData);
                                    }
                                  }}
