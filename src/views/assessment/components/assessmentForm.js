...
const AssessmentForm = () => {
  ...
  const CreateQuiz = async (apiData) => {
    try {
      setIsLoading(true);
      const response = await axiosInstance.post(`user/quiz`, apiData);
  ...
  const CreateExercise = async (apiData) => {
    try {
      setIsLoading(true);
      ...
      console.log("CreateExercise error: ", error);
      openNotification("danger", error.message);
    } finally {
      setIsLoading(false);
    }
        ...
        <>
          <Card bodyClass="p-3 sm:p-[1.25rem]">
            <div className="flex justify-between items-center ">
              <div
                className={`text-xl mb-2 mx-2 capitalize font-semibold text-${themeColor}-${primaryColorLevel}`}
              >
                {sectionData?.title}
              </div>
            </div>
            ...
            >
              ...
              <div
                className={`mt-4 p-2 flex justify-center text-${themeColor}-${primaryColorLevel} border-2 border-dashed border-gray-400 rounded-lg  bg-gray-50`}
              >
                <div>
                  {sectionData?.type === "quiz" && (
                    <Button
                      size="md"
                      variant="plain"
            ...
            <div className="max-h-96 overflow-y-auto hidden-scroll">
              ...
              <div className="col-span-1 gap-4 mb-4">
                ...
                <div className="col-span-2">
                  <Input
                    type="text"
                    placeholder="Please Enter Exercise Title"
                    onChange={(e) => {
                      setFormData({
                        ...formData,
                        title: e.target.value
